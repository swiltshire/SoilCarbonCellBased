---
title: "Cell based RothC with climate projection data"
output:
  html_document:
    df_print: paged
---
...
params_ls <- list(cell_id = 40604, # cell number to use
                  lu_labs = "crops", #c("crops", "hay", "pasture"),
                  gcm = "mri-cgcm3",
                  soil_thick = 30,
                  t0_yr = 2022,
                  n_yrs_init = 750,
...
Initialize timestep vectors and list objects for input data and results.
...
pop_gis_dat_ls <- function(cell_id, gis_dat_path) {
  ...
  gis_dat_ls <- list() # init gis data list
  ...
  # record areas for crops, hay, pasture
  # note: NLCD dataset does not differentiate between hay and pasture
  hay_pasture <- gis_dat_row$nlcd_Hay_Pasture * 100 # convert sq. km. to Ha
  
  gis_dat_ls$area <- list(crops = gis_dat_row$nlcd_Cultivated_Crops * 100, # convert sq. km. to Ha
                           hay = hay_pasture * 0.513, # census of ag. VT avg. percent of grassland used for hay vs. forage
                           pasture = hay_pasture * 0.487,
                           total = gis_dat_row %>% select(contains("nlcd")) %>% unlist(use.names = F) %>% sum(na.rm = T) * 100
  ) 
  
  gis_dat_ls$area[is.na(gis_dat_ls$area)] <- 0
  
  gis_dat_ls
}

gis_dat_ls <- pop_gis_dat_ls(params_ls$cell_id, "input_data/cell_gis_dat_all.xlsx")
...
pop_gcm_dat_ls <- function(cell_id, gcm) {
      ...
      gcm_dat <- read_csv(paste("input_data/", gcm, "_1950-2099_by_cell/", cell_id, ".csv", sep = ""), show_col_types = F)
...
lm_dat_ls <- pop_lm_dat_ls("input_data/rothc_lm_in_dat.xlsx")
...
# run spinup optimization

for (lu_lab in params_ls$lu_labs) {
  print(paste("*** Calibrating below-ground C, cell #", params_ls$cell_id, ", ", lu_lab, sep = ""))
...
for (lu_lab in params_ls$lu_labs) {
  ...
  print(paste("Running final spinup, cell #", params_ls$cell_id, ", ", lu_lab, sep = ""))
...
# run the simulations
for (lu_lab in params_ls$lu_labs) {
  print(paste("Running sims, cell #", params_ls$cell_id, ", ", lu_lab, sep = ""))
...
Plot with avg and sd temp
```{r}


win_size_mos <- 120

comb_tmp_plt_dat <- tibble()

for (er_code in params_ls$er_codes) {
  
  tmp_plt_dat <- tibble("Year" = rollmean(head(seq(1950, 2099, by = 1/12), -1), win_size_mos))
  
  for (gcm in params_ls$gcms) {
    tmp_plt_dat[gcm] <- rollmean(gcm_dat_ls[[er_code]][[gcm]]$Tavg, win_size_mos)
    
    # sanity check historic averages
    print(paste(gcm, "historical avg. temp 1950-2022 =", mean(head(gcm_dat_ls[[er_code]][[gcm]]$Tavg, 72*12))))

  }
  
    comb_tmp_plt_dat <- bind_rows(comb_tmp_plt_dat,
                                tmp_plt_dat %>%
                                  mutate(er_code = er_code))
  
  p <- tmp_plt_dat %>%
    pivot_longer(cols = -"Year", names_to = "GCM", values_to = "Temperature") %>%
    ggplot(aes(x = Year, y = Temperature, color = GCM)) +
    geom_line() +
    theme_minimal()
...
comb_tmp_plt_dat <- comb_tmp_plt_dat %>%
  group_by(Year) %>%
  summarise_if(is.numeric, mean) %>%
  mutate(mean = rowMeans(select(., -Year)), sd = rowSds(as.matrix(select(., -Year)))) %>%
  select(c(Year, mean, sd)) 
...
p <- comb_tmp_plt_dat %>%
  ...
  geom_line() +
  geom_vline(xintercept = params_ls$t0_yr, color = "darkred", size = .6, linetype = "longdash") +
  labs(x = "Year", y = "Temperature (°C)") +
  scale_x_continuous(limits = c(NA, NA), expand = expansion(mult = c(.01, 0)), breaks = pretty_breaks(10)) +
  scale_y_continuous(expand = expansion(mult = c(.01, .01)), breaks = pretty_breaks(5)) +
  # ggtitle("Mean Vermont temperature from EPSCoR GCM data") +
  theme_ipsum_rc() +
...
ggsave("output_figs/vt_temp_line.png", width = 7, height = 5)
ggsave("output_figs/vt_temp_line.pdf", device = cairo_pdf, width = 7, height = 5)
