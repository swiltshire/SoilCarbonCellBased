...
rothc_by_cell <- function(cell_id, gcm, gis_dat_path, gcm_dat_path, lm_dat_path, out_dat_path) {
  ...
  params_ls <- list(cell_id = cell_id,
                    ...
                    lu_labs = "pasture",#c("crops", "hay", "pasture"),
                    t0_yr = 2022,
                    n_yrs_init = 750,
  ...
  fit_fxn <- function (bgc, lu_lab, gis_dat_ls, lm_dat_ls, scn_dat_ls, params_ls) {
    ...
    print(paste("Cell #", params_ls$cell_id, ", ", lu_lab, 
                ", Empirical = ", round(soc_mg_ha_emp, digits = 2), 
                "; Modeled = ", round(soc_mg_ha_spinup, digits = 2), sep = ""))
  ...
  for (lu_lab in params_ls$lu_labs) {
    ...
    if (bgc_opt$objective > 1) {
      print(paste("Cell #", params_ls$cell_id, ", ", lu_lab, ", did not converge during spinup: diff = ", round(bgc_opt$objective, digits = 2), sep = ""))
    }
